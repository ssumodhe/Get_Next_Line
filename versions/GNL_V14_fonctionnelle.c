...
/*   Updated: 2017/02/09 21:13:00 by ssumodhe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//	char		buff[BUFF_SIZE + 1];
	...
	*line = NULL;
	...
	if (clem != NULL)
	{
//		printf("Il reste de la lecture precedente\n");
		if (buff_stock)
		{
		...
		if (buff_stock != NULL)
		{
			...
			if (ft_analysis(buff_stock) == 1)
			{
				...
				i = 0;
				while (buff_stock[i] != '\n' && buff_stock[i] != '\0') //need '\0' ???
					i++;
				if ((buff_start = (char *)malloc(sizeof(char) * (i + 1))) == NULL)
					return (-1);
				i = 0;
				while (buff_stock[i] != '\n')
				{
				...
				if (buff_stock[i + 1]) // && buff_stock[i + 1] != '\0' ??
...
//		printf("On a un pas de \\n\n");
		...
		{
//		printf("On va devoir remplir line\n");
			i = 0;
			while (buff[i] != '\n')
				i++;
			if ((buff_start = (char *)malloc(sizeof(char) * (i + 1))) == NULL)
			...
			if (tmp)
			{
			/*	printf(HIGHLIGHT "En effet je passe ici\n" RESET);
				printf("Tmp : <%s>\n", tmp);
				printf("Buff_start : <%s>\n", buff_start);*/
			//	ft_putendl("ICI");
	...
	if (buff_stock != NULL)
	{
//		printf("On est dans la derniere ligne / derniere lecture\n");
